from lib.Variables import Variables
import datetime
from lib.Logger import Logger
from lib.SnowflakeConnector import SnowFlakeConnect
from lib.BatchMonitor import BatchMonitor

def order_load():
    v = Variables()
    v.set("SCRIPT_NAME", 'Order_ld')
    v.set("TABLE","ORDERS")
    v.set("TEMP_TABLE","TMP_ORDERS_F")
    v.set("REJECTION_TABLE","REJ_ORDERS_F")
    v.set("TARGET_TABLE","DWH_ORDERS_F")
    log = Logger(v)
    sqls = SnowFlakeConnect(v, log)
    sqls.startConnection()
    monitor=BatchMonitor(sqls,v,log,'LOAD')
    try:
        monitor.start_audit()
        if v.get("BOOKMARK")=="NULL":
            print("Order Loading Started " + str(datetime.datetime.now()))
            log.log_message("Order Loading Started " + str(datetime.datetime.now()))

            log.log_message("LOADING THE STAGE TABLE FROM STAGE FILE")
            log.log_message("------------*****STARTED******-----------")

            sqls.load_stage_from_file(v.get("TABLE"))
            log.log_message(f"Stage table Load Completed Sucessfully ")

            log.log_message("LOADING THE STAGE TABLE FROM STAGE FILE")
            log.log_message("------------*****COMPLETED******-----------")

            monitor.set_bookmark("AFTER_STG_LOAD")
        if v.get("BOOKMARK") == "AFTER_STG_LOAD":
            log.log_message("LOADING THE TEMP TABLE FROM STAGE TABLE")
            log.log_message("------------*****STARTED******-----------")

            log.log_message("Temp table load  Started " + str(datetime.datetime.now()))
            sqls.truncate_table(v.get("DATABASE"),v.get("TEMP_SCHEMA"), v.get("TEMP_TABLE"))

            temp_load=f"""INSERT INTO {v.get ("DATABASE") }.{v.get ("TEMP_SCHEMA") }.{v.get ("TEMP_TABLE") }
                            SELECT ORDER_NUM  ,
                            PARENT_ORDER_NUM ,
                            ORDER_TYPE ,
                            DECODE(ORDER_TYPE,'COMPANY_PO',COMP_BUYER.CU_ID ,CUST_BUYER.DO_KEY ),
                            BUYER ,
                            COMP_SELLER.CU_KEY ,
                            SELLER ,
                            PROD_KEY ,
                            SRC.PROD_ID ,
                            ORDERED_BY ,
                            QTY ,
                            SRC.UNIT_PRICE ,
                            QTY*SRC.UNIT_PRICE TOTAL_AMOUNT ,
                            ORDER_DATE ,
                            SHIPPING_ADDRESS ,
                            DELIVERY_CONTACT ,
                            DELIVERY_STATUS ,
                            DELIVERED_DATE 
                            FROM {v.get("DATABASE") }.{v.get("SOURCE_SCHEMA") }.{v.get("TABLE") } SRC
                            LEFT JOIN {v.get("DATABASE") }.{v.get("TARGET_SCHEMA") }.DWH_PRODUCT_D PROD ON (SRC.PROD_ID=PROD.PROD_ID)
                            LEFT JOIN {v.get("DATABASE") }.{v.get("TARGET_SCHEMA") }.DWH_COMPANY_D COMP_BUYER ON (SRC.BUYER=COMP_BUYER.CU_ID and SRC.ORDER_TYPE like 'COMPANY_PO')
                            LEFT JOIN {v.get("DATABASE") }.{v.get("TARGET_SCHEMA") }.DWH_CUSTOMER_D CUST_BUYER ON (SRC.BUYER=CUST_BUYER.DO_ID and SRC.ORDER_TYPE like 'COMPANY_SALES')
                            LEFT JOIN {v.get("DATABASE") }.{v.get("TARGET_SCHEMA") }.DWH_COMPANY_D COMP_SELLER ON (SRC.SELLER=COMP_SELLER.CU_ID)"""

            result =sqls.load_table(temp_load, v.get("TEMP_TABLE"), v.get("TEMP_SCHEMA"))
            log.log_message("Temp table load  Completed " + str(datetime.datetime.now()))

            log.log_message("LOADING THE TEMP TABLE FROM STAGE TABLE")
            log.log_message("------------*****COMPLETED******-----------")
            monitor.set_bookmark("AFTER_TMP_LOAD")

        if v.get("BOOKMARK") == "AFTER_TMP_LOAD":
            log.log_message("LOADING THE REJECTION TABLE FROM TEMP TABLE")
            log.log_message("------------*****STARTED******-----------")

            log.log_message("Rejection table load  Started " + str(datetime.datetime.now()))
            sqls.truncate_table(v.get("DATABASE"), v.get("REJECTION_SCHEMA"), v.get("REJECTION_TABLE"))

            temp_load = f"""INSERT INTO {v.get("DATABASE")}.{v.get("REJECTION_SCHEMA")}.{v.get("REJECTION_TABLE")}
                                    SELECT *
                                    FROM {v.get ("DATABASE") }.{v.get ("TEMP_SCHEMA") }.{v.get ("TEMP_TABLE") } 
                                    where (BUYER_KEY is null OR PROD_KEY is null OR SELLER_KEY is NULL OR ORDER_NUM is null)"""


            sqls.load_table(temp_load, v.get("REJECTION_TABLE"), v.get("REJECTION_SCHEMA"))
            log.log_message("Rejection table load  Completed " + str(datetime.datetime.now()))

            log.log_message("LOADING THE REJECTION TABLE FROM TEMP TABLE")
            log.log_message("------------*****COMPLETED******-----------")
            monitor.set_bookmark("AFTER_REJ_LOAD")

        if v.get("BOOKMARK") == "AFTER_REJ_LOAD":
            log.log_message("UPDATING THE TARGET TABLE FROM TEMP TABLE")
            log.log_message("------------*****STARTED******-----------")
            update_tgt_table = f""" UPDATE {v.get("DATABASE")}.{v.get("TARGET_SCHEMA")}.{v.get("TARGET_TABLE")} AS T1
                                                       SET 	T1.PARENT_ORDER_NUM = T2.PARENT_ORDER_NUM
                                                        ,T1.ORDER_TYPE = T2.ORDER_TYPE
                                                        ,T1.BUYER = T2.BUYER
                                                        ,T1.SELLER = T2.SELLER
                                                        ,T1.PROD_ID = T2.PROD_ID
                                                        ,T1.ORDERED_BY = T2.ORDERED_BY
                                                        ,T1.QTY = T2.QTY
                                                        ,T1.UNIT_PRICE = T2.UNIT_PRICE
                                                        ,T1.TOTAL_AMOUNT = T2.TOTAL_AMOUNT
                                                        ,T1.ORDER_DATE = T2.ORDER_DATE
                                                        ,T1.SHIPPING_ADDRESS = T2.SHIPPING_ADDRESS
                                                        ,T1.DELIVERY_CONTACT = T2.DELIVERY_CONTACT
                                                        ,T1.DELIVERY_STATUS = T2.DELIVERY_STATUS
                                                        ,T1.DELIVERED_DATE = T2.DELIVERED_DATE
                                                       ,RCD_UPDATE_DT = LOCALTIMESTAMP
                                                       FROM {v.get ("DATABASE") }.{v.get ("TEMP_SCHEMA") }.{v.get ("TEMP_TABLE") } AS T2
                                                       WHERE T1.ORDER_NUM = T2.ORDER_NUM
                                                       AND(NVL(T1.PARENT_ORDER_NUM, '0') <> NVL(T2.PARENT_ORDER_NUM, '0')
                                                        OR NVL(T1.ORDER_TYPE, '?') <> NVL(T2.ORDER_TYPE, '?')
                                                        OR NVL(T1.BUYER, '0') <> NVL(T2.BUYER, '0')
                                                        OR NVL(T1.SELLER, '0') <> NVL(T2.SELLER, '0')
                                                        OR NVL(T1.PROD_ID, '0') <> NVL(T2.PROD_ID, '0')
                                                        OR NVL(T1.ORDERED_BY, '0') <> NVL(T2.ORDERED_BY, '0')
                                                        OR NVL(T1.QTY, '0') <> NVL(T2.QTY, '0')
                                                        OR NVL(T1.UNIT_PRICE, '0') <> NVL(T2.UNIT_PRICE, '0')
                                                        OR NVL(T1.TOTAL_AMOUNT, '0') <> NVL(T2.TOTAL_AMOUNT, '0')
                                                        OR NVL(T1.ORDER_DATE, '?') <> NVL(T2.ORDER_DATE, '?')
                                                        OR NVL(T1.SHIPPING_ADDRESS, '?') <> NVL(T2.SHIPPING_ADDRESS, '?')
                                                        OR NVL(T1.DELIVERY_CONTACT, '?') <> NVL(T2.DELIVERY_CONTACT, '?')
                                                        OR NVL(T1.DELIVERY_STATUS, '?') <> NVL(T2.DELIVERY_STATUS, '?')
                                                        OR NVL(T1.DELIVERED_DATE, '?') <> NVL(T2.DELIVERED_DATE, '?'))
                                                       ;
                               """
            sqls.load_table(update_tgt_table, v.get("TARGET_TABLE"), v.get("TARGET_SCHEMA"))

            log.log_message("Updating target table from Temp Completed " + str(datetime.datetime.now()))

            log.log_message("UPDATING THE TARGET TABLE FROM TEMP TABLE")
            log.log_message("------------*****COMPLETED******-----------")
            monitor.set_bookmark("AFTER_TARGET_UPDATE")


        if v.get("BOOKMARK")=="AFTER_TARGET_UPDATE":
            log.log_message("INSERTING THE TARGET TABLE FROM TEMP TABLE")
            log.log_message("------------*****STARTED******-----------")
            load_tgt_table = f"""INSERT INTO {v.get("DATABASE")}.{v.get("TARGET_SCHEMA")}.{v.get("TARGET_TABLE")}(
                            ORDER_NUM
                            ,PARENT_ORDER_NUM 
                            ,ORDER_TYPE 
                            ,BUYER_KEY 
                            ,BUYER 
                            ,SELLER_KEY 
                            ,SELLER 
                            ,PROD_KEY 
                            ,PROD_ID 
                            ,ORDERED_BY 
                            ,QTY 
                            ,UNIT_PRICE 
                            ,TOTAL_AMOUNT 
                            ,ORDER_DATE 
                            ,SHIPPING_ADDRESS 
                            ,DELIVERY_CONTACT 
                            ,DELIVERY_STATUS 
                            ,DELIVERED_DATE 
                            ,RCD_INSERT_DT
                            ,RCD_UPDATE_DT
                        ) SELECT
                            ORDER_NUM
                            ,PARENT_ORDER_NUM 
                            ,ORDER_TYPE 
                            ,BUYER_KEY 
                            ,BUYER 
                            ,SELLER_KEY 
                            ,SELLER 
                            ,PROD_KEY 
                            ,PROD_ID 
                            ,ORDERED_BY 
                            ,QTY 
                            ,UNIT_PRICE 
                            ,TOTAL_AMOUNT 
                            ,ORDER_DATE 
                            ,SHIPPING_ADDRESS 
                            ,DELIVERY_CONTACT 
                            ,DELIVERY_STATUS 
                            ,DELIVERED_DATE 
                            ,LOCALTIMESTAMP
                            ,LOCALTIMESTAMP
                        FROM {v.get ("DATABASE") }.{v.get ("TEMP_SCHEMA") }.{v.get ("TEMP_TABLE") }
            WHERE ORDER_NUM NOT IN (SELECT ORDER_NUM from {v.get("DATABASE")}.{v.get("TARGET_SCHEMA")}.{v.get("TARGET_TABLE")} )
            AND (BUYER_KEY is not null  AND SELLER_KEY is not NULL AND PROD_KEY is not null AND ORDER_NUM is not null);"""
            #

            sqls.load_table(load_tgt_table, v.get("TARGET_TABLE"), v.get("TARGET_SCHEMA"))

            log.log_message("INSERTING THE TARGET TABLE FROM TEMP TABLE")
            log.log_message("------------*****COMPLETED******-----------")
            print("Order Loading Finished " + str(datetime.datetime.now()))
            log.log_message("Order Loading Finished " + str(datetime.datetime.now()))
            monitor.audit_complete()

    except Exception as e:
        print("Order Loading Failed",e)
        log.log_message("Order Loading Failed " + str(datetime.datetime.now()))
        log.log_message(f"ERROR: {e}")
        monitor.script_error()
        raise e

    finally:
        sqls.end_connection()
        log.close()
